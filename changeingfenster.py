...
def is_valid_email(email):
    # Verwendet eine einfache Regular Expression,
    # um die Gültigkeit der E-Mail-Adresse zu überprüfen
...
def change_button_action():
    ...
    if (
        ...
        if not is_valid_email(email_input):
            ...
            (message_label.config
             (text="Ungültiges Geburtsdatum. Bitte im Format dd.mm.yyyy "
                   "eingeben."))
        elif not is_valid_name(vorname_input):
            (message_label.config
             (text="Ungültiger Vorname. Bitte nur Buchstaben verwenden."))
        elif not is_valid_name(nachname_input):
            (message_label.config
             (text="Ungültiger Nachname. Bitte nur Buchstaben verwenden."))
        elif not is_valid_name(ort_input):
            (message_label.config
             (text="Ungültiger Wohnort. Bitte nur Buchstaben verwenden."))
...
vorname_label = Label(changefenster, text="Vorname : ",
...
nachname_label = Label(changefenster, text="Nachname : ",
                       anchor="w", justify=LEFT)
...
geburtsdatum_label = Label(changefenster, text="Geburtsdatum : ",
                           anchor="w", justify=LEFT)
...
id_label = Label(changefenster, text="Kundenummer : ",
                 anchor="w", justify=LEFT)
id_label.place(x=20, y=220, width=100, height=20)
id_label_ausgabe = Label(changefenster, text=ausgabedaten["id"],
                         anchor="w", justify=LEFT)
...
back_button = Button(changefenster, text="<-Zurück",
                     command=back_button_action)
back_button.place(x=20, y=260, width=70, height=20)
change_button = Button(changefenster, text="Bearbeitung Speichern",
