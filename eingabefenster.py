...
def is_valid_email(email):
    # Verwendet eine einfache Regular Expression, um die Gültigkeit
    # der E-Mail-Adresse zu überprüfen
    pattern = (r'[a-zA-Z0-9ß]*[._-]?[a-zA-Z0-9ß]*[._-]?[a-zA-Z0-9ß]@'
               r'[a-zA-Z0-9ß]+[._-]?[a-zA-Z0-9ß]*[._-]?[a-zA-Z0-9ß]*\.'
               r'[a-zA-Z0-9ß]{2,}$')
...
def is_valid_date(date):
    ...
    if re.match(date_pattern, date) is not None:
        # Das Datum hat das richtige Format, überprüfe,
        # ob es in der Zukunft liegt
...
def anlegen_button_action():
    ...
    if (
        ...
        kunde = Kunde(vorname_input, nachname_input, geburtsdatum_input,
                      email_input, ort_input)
        with open("Kundendaten.json", "a",  encoding="utf-8") as appender:
            json.dump(kunde.__dict__, appender, ensure_ascii=False)
            appender.write("\n")
        message_label.config(text=f"Ein neuer Kunde mit der ID {kunde.id} "
                                  f"wurde hinzugefügt")
        ...
        if not is_valid_email(email_input):
            (message_label.config
             (text="Ungültige E-Mail-Adresse. Bitte erneut eingeben."))
        elif not is_valid_date(geburtsdatum_input):
            message_label.config(text="Ungültiges Geburtsdatum. "
                                      "\nFalsches format (dd.mm.yyyy) oder "
                                      "das Datum liegt in der Zukunft.")
        elif not is_valid_name(vorname_input):
            message_label.config(text="Ungültiger Vorname. "
                                      "Bitte nur Buchstaben verwenden.")
        elif not is_valid_name(nachname_input):
            message_label.config(text="Ungültiger Nachname. "
                                      "Bitte nur Buchstaben verwenden.")
        elif not is_valid_name(ort_input):
            message_label.config(text="Ungültiger Wohnort. "
                                      "Bitte nur Buchstaben verwenden.")
        elif not is_email_unique(email_input):
            message_label.config(text="Die E-Mail-Adresse wird bereits "
                                      "verwendet. Bitte eine andere "
                                      "verwenden.")
...
vorname_label = Label(eingabefenster, text="Vorname : ", anchor="w",
                      justify=LEFT)
...
nachname_label = Label(eingabefenster, text="Nachname : ", anchor="w",
                       justify=LEFT)
...
geburtsdatum_label = Label(eingabefenster, text="Geburtsdatum : ", anchor="w",
                           justify=LEFT)
...
anlegen_button = Button(eingabefenster, text=" Als neuen Kunden anlegen ",
                        command=anlegen_button_action)
anlegen_button.place(x=100, y=220, width=380, height=20)

message_label = Label(eingabefenster, text="")
message_label.place(x=100, y=260, width=380, height=40)

back_button = Button(eingabefenster, text="<-Zurück",
